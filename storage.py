...
class Volume:
    ...
    def formatVolume(inputs,cot_format = 0): # type: ignore
        volidx = [
            "",
            "K",
            "M",
        ...
        if cot_format:
            ...
            output = buff[-1][::-1] + "." + buff[-2][::-1][:2] + " " + (volidx[len(buff)-1] if len(buff)-1 < len(volidx) else f'e^{len(buff1)-1}') + "b"
        return output
...
class Encrypt:
    ...
    def newkey(self): return Fernet.generate_key()

class Storage:
    def __init__(self,st_path:str = '',starting_path:str = '/'):
        self.spath = st_path
    ...
    def newStorage(self,filename:str = '',headsize:int = 128,metasize:int = 8):
        header,metadata,key = [0 for _ in range(headsize//4)],dict(DEFAULT_METADATA),Fernet.generate_key()

        fernet = Encrypt(key)

        header[0] = headsize
        header[1] = len(key)
        header[2] = Volume.KiloBytes(metasize)
        header[3] = len(json.dumps(metadata))

        if not len(filename):
            filename = f'storage-{key[:6].decode()}.sxa'

        ## PACKING
        with open(filename,'wb') as fs:
            buff,header = header,b''
            for x in buff: header += int(x).to_bytes(4,'big')
            fs.write(self.fillEmpty(header,headsize))
            fs.write(key)
            fs.write(self.fillEmpty(fernet.encrypt(json.dumps(metadata).encode()),Volume.KiloBytes(metasize)))
            fs.close()
